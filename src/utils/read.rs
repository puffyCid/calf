...
pub(crate) fn read_bytes<T: std::io::Read + std::io::Seek>(
    ...
    if fs.seek(SeekFrom::Start(offset)).is_err() {
        event!(Level::ERROR, "[calf] Could not seek to offset {offset}");
    ...
    let bytes_read = match fs.read(&mut buff_size) {
        Ok(result) => result,
        Err(err) => {
            event!(Level::ERROR, "[calf] Could not read bytes: {err:?}");
    ...
    if bytes_read != buff_size.len() {
        event!(
            Level::WARN,
            "[calf] Did not read expected number of bytes. Wanted {bytes} got {bytes_read}",
        );
    }
